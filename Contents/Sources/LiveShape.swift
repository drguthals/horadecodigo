...
public class LiveShape: VirtualObject {
    ...
    private func updateMaterials() {
        ...
        switch shapeType {
        case .cube, .pyramid:
            // Cube: apply the image to the vertical faces.
            // Pyramid: apply the image to the sloping faces.
            for i in 0..<min(4, geometry.materials.count) {
                geometry.materials[i].diffuse.contents = image
            }
        case .sphere:
