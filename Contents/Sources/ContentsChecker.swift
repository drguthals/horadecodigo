...
public class ContentsChecker {
    ...
    public init(contents: String) {
        ...
        do {
            ...
            nodes = []
            NSLog("Contents Parser error: \(error).")
        }
    ...
    public var didUseForLoop: Bool {
        return loopNodes.contains {
            $0.type == .for
        }
    }
    
    public var didUseWhileLoop: Bool {
        return loopNodes.contains {
            $0.type == .while
        }
    }
    ...
    /// Returns `true` if a function was defined and then called at least once.
    public func didCallCustomFunction() -> Bool {
